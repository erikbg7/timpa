<script lang="ts">
	...
	import P from '$lib/markdown/p.svelte';
	...
	<div slot="content">
		{#if $page.data?.props?.plan === Plan.FREE}
			...
			<p class="my-8 text-sm">
				Your are missing out some <a class="text-accent" href="/pricing">key features</a> to discover
				and unlock you true potential.
			</p>
		...
		{#if $page.data?.props?.plan === Plan.STANDARD}
			...
			<p class="my-8 text-sm">
				Your are missing out some <a href="/pricing">key features</a> to discover and unlock you true
				potential.
			</p>
			<div class="relative flex items-center justify-between rounded-md bg-accent-content p-6">
				<DecorationCard />
